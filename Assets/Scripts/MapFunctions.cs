...
public class MapFunctions
{
    ...
    public static void RenderMap(int[,] map, Tilemap tilemap, TileBase tile, TileBase left, TileBase right, TileBase top)
    {
        ...
        for (int x = 0; x < map.GetUpperBound(0) ; x++) //Loop through the width of the map
        {
            ...
            {
                if (map[x, y] == 1) // 1 = tile, 2 = left, 3 = right, 4 = top, 0 = no tile
                {
	...
	/// <param name="mapSettingTopLayerOnly"></param>
	/// <returns>The modified map with a smoothed random walk</returns>
	public static int[,] RandomWalkTopSmoothed(int[,] map, float seed, int minSectionWidth, bool mapSettingTopLayerOnly)
    {
        //Seed our random
        System.Random rand = new System.Random(seed.GetHashCode());
        ...
        for (int x = 0; x <= map.GetUpperBound(0); x++)
        {
	        ...
	        if (x == 0)
	        {
		        for (int y = lastHeight + 1; y < map.GetUpperBound(1); y++)
		        {
			        map[x, y] = 2;
		        }
